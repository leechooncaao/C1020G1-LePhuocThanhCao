interface ISingleRepo {
  ...
  async function fetchRepo(keyword : string): Promise<Array<ISingleRepo>> {
    let res: Response | IRepos = await fetch('https://api.github.com/search/repositories?q=' + keyword);
  ...
  async function main(keyword : string) {
    ...
    // step 1: fetch repo
    const res = await fetchRepo(keyword);
